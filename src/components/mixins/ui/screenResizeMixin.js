		...
		watchResize: function() {
			...
			this.resizeTimer = setTimeout(function() {
				...
				if(newScreenWidth <= 768){
					// Is mobile 
					_this.$store.getters.device.isMobile = true;

					// Now check if keyboard is visible
					var softKeyboardOn = _this.$store.getters.device.softKeyboard;
					...
					if(changePercent > 20 && !softKeyboardOn){
						...
						_this.$store.getters.device.softKeyboard = true;
					}else if(softKeyboardOn && changePercent < 20){
						// If keyboard is open, hide it
						softKeyboardOn = false;
						_this.$store.getters.device.softKeyboard = false;
					}
				}else{
					_this.$store.getters.device.softKeyboard = false;
					_this.$store.getters.device.isMobile = false;
				}
